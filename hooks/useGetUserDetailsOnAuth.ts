...
function useGetUserDetailsOnAuth() {
  ...
  function userLiveUpdates(user: User) {
    // Vérifier que le displayName existe
    if (!user.displayName) {
      console.error('DisplayName not set');
      return;
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (document: any) => {
        ...
        if (userData) {
          ...
          getChatRoomMessages(userData);
          getHomePagePosts(userData);
          getFollowingStories(userData);
        }
      }
    );
    setListeners((current) => [...current, unsubscribe]);
  ...
  React.useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      // Attendre que displayName soit défini
      ...
      return () => clearInterval(checkDisplayName);
    });
