...
const Inbox: NextPage = () => {
  ...
  const [hasTriedToOpenChat, setHasTriedToOpenChat] = React.useState(false);

  // Dans Inbox.tsx
  React.useEffect(() => {
    if (!userNotifications.chatRoomIds || hasTriedToOpenChat) return;

    const chatRoomToOpen = localStorage.getItem('openChatRoom');
    if (chatRoomToOpen) {
      const chatRoomIndex = userNotifications.chatRoomIds.findIndex(
        id => id === chatRoomToOpen
      );

      if (chatRoomIndex !== -1) {
        console.log('Opening chat:', chatRoomIndex); // Debug
        setActiveChat(`chatRoom${chatRoomIndex}`);
        handleResetNewMessage({
          username: userDetails.displayName!,
          chatRoomId: chatRoomToOpen
        });
      }
      localStorage.removeItem('openChatRoom');
      setHasTriedToOpenChat(true);
    }
  }, [userNotifications.chatRoomIds, userDetails.displayName, hasTriedToOpenChat]);
