import { doc, setDoc, updateDoc, getFirestore, serverTimestamp, arrayUnion } from 'firebase/firestore';
...
const Profile: NextPage = () => {
  ...
  const handleMessageClick = async () => {
    if (isCreatingChat) return; // Évite les doubles clics
    setIsCreatingChat(true);

    try {
      const db = getFirestore(app);
      const chatRoomId = userNotifications.userId! + profileNotifications.userId;
      const reverseChatRoomId = profileNotifications.userId! + userNotifications.userId;
      ...
      if (!hasExistingChat) {
        ...
        await setDoc(
          doc(db, chatRoomId, 'users'),
          {
            ...
            [`${profileNotifications.username}ChatName`]: userNotifications.username,
            [`${profileNotifications.username}Avatar`]: profileNotifications.avatarURL,
            [`${profileNotifications.username}NewMessage`]: false,
            [`${userNotifications.username}ChatName`]: profileNotifications.username,
        ...
        await Promise.all([
          ...
          updateDoc(doc(db, 'users', profileNotifications.username!), {
            chatRoomIds: arrayUnion(chatRoomId)
          })
        ]);

        // Stocke temporairement l'ID de la conversation à ouvrir
        localStorage.setItem('openChatRoom', chatRoomId);
      } else {
        // Stocke l'ID de la conversation existante
    ...
    <div className="h-[100vh] overflow-y-scroll bg-[#fafafa] text-[#231f20] dark:bg-[#131313] dark:text-slate-100 dark:[color-scheme:dark]">
      ...
      <div className="mx-auto  max-w-[935px] pt-6 sm:pt-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center">
              ...
              {nameSearch === profileDetails.displayName ? (
                ...
                <div className="flex flex-row items-center pb-2 sm:pl-7 sm:pb-0">
                  ...
                  >
                    {isCreatingChat ? 'Chargement...' : 'Message'}
                  </button>
        ...
        <div>
          ...
          <div
            className={`${userPorfileLoading ? 'fixed opacity-0' : ''
